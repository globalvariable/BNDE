...
static int exp_ctrl_2_neural_net_msgs_handler_rt_thread;

static void *snn_rt_handler(void *args);
static void *exp_ctrl_2_neural_net_msgs_handler(void *args); 
...
void create_snn_rt_threads(void)
{
	...
	if (first)
	{
		...
		exp_ctrl_2_neural_net_msgs_handler_rt_thread =  rt_thread_create(exp_ctrl_2_neural_net_msgs_handler, NULL, 10000);
...
static void *snn_rt_handler(void *args)
{
	...
	unsigned int num_of_rxed_spikes, j;
	...
	integration_start_time = (current_snn_time/NEURON_NODE_EVALUATING_INTERVAL) *NEURON_NODE_EVALUATING_INTERVAL;

	reset_all_network_iz_neuron_dynamics (in_silico_network);	// clears neuron synaptic event buffers as well

	msgs_prosthetic_ctrl_2_neural_net = msgs_prosthetic_ctrl_2_neural_net_multi_thread[task_num];

	msgs_prosthetic_ctrl_2_neural_net->buff_read_idx = msgs_prosthetic_ctrl_2_neural_net->buff_write_idx;   // to reset message buffer. previously written messages and reading of them now might lead to inconvenience.,
...
//						printf("rew = %.3f\n", reward);
				...
				case PROSTHETIC_CTRL_2_NEURAL_NET_MSG_START_TRIAL:
					...
					break;	
				default:
					print_message(BUG_MSG ,"HybridNetRLBMI", "HybridNetRLBMI", "prosthetic_ctrl_2_neural_net_msgs_handler", "Invalid message.");	
					goto EXIT;
			}
		}		

		integration_end_time =  (current_snn_time/NEURON_NODE_EVALUATING_INTERVAL) * NEURON_NODE_EVALUATING_INTERVAL;
		for (time_ns = integration_start_time; time_ns < integration_end_time; time_ns+= NEURON_NODE_EVALUATING_INTERVAL)   // integrate remaining part in the next task period
		{
			...
			{
				nrn = all_neurons[i];
				if (! evaluate_node_inputs (nrn, &num_of_rxed_spikes, time_ns, time_ns+NEURON_NODE_EVALUATING_INTERVAL)) { 
					print_message(ERROR_MSG ,"HybridNetRLBMI", "HybridNetRLBMIRtTask", "snn_rt_handler", "! evaluate_node_inputs()."); exit(1); }	

				for (j=0; j<num_of_rxed_spikes; j++)
				{
					if (! write_to_neural_net_2_prosthetic_ctrl_msg_buffer(msgs_neural_net_2_prosthetic_ctrl_multi_thread[task_num], integration_start_time, NEURAL_NET_2_PROSTHETIC_CTRL_MSG_SPIKE_OUTPUT, nrn->layer, nrn->neuron_group, nrn->neuron_num, time_ns))  // we assume all spikes are generated at time_ns 
					{
						print_message(ERROR_MSG ,"HybridNetRLBMI", "HybridNetRLBMIRtTask", "snn_rt_handler", "! write_to_neural_net_2_prosthetic_ctrl_msg_buffer().");
						goto EXIT;
					}					
				}
...
/*				if (! evaluate_neuron_dyn_pre_post_w_binary_stdp_elig(nrn, time_ns, time_ns+PARKER_SOCHACKI_INTEGRATION_STEP_SIZE, &spike_generated, &spike_time)) {
				...
				if (spike_generated)
				{
					...
					{
						if (! write_to_neural_net_2_prosthetic_ctrl_msg_buffer(msgs_neural_net_2_prosthetic_ctrl_multi_thread[task_num], integration_start_time, NEURAL_NET_2_PROSTHETIC_CTRL_MSG_SPIKE_OUTPUT, nrn->layer, nrn->neuron_group, nrn->neuron_num, spike_time)) 
						{
							print_message(ERROR_MSG ,"HybridNetRLBMI", "HybridNetRLBMIRtTask", "snn_rt_handler", "! write_to_neural_net_2_prosthetic_ctrl_msg_buffer().");
							goto EXIT;
						}
...
static void *exp_ctrl_2_neural_net_msgs_handler(void *args)
{
	...
	char str_exp_ctrl_msg[EXP_CTRL_2_NEURAL_NET_MSG_STRING_LENGTH];
	...
	trial_2_nn_time_ns =  rt_get_time_ns_cpuid(timer_cpuid);	// use system time
        ...
        period = nano2count(EXP_CTRL_2_NEURAL_NET_MSGS_HANDLER_PERIOD);
        rt_task_make_periodic(handler, curr_time + period, period);
	...
	msgs_exp_ctrl_2_neural_net->buff_read_idx = msgs_exp_ctrl_2_neural_net->buff_write_idx; // to reset message buffer. previously written messages and reading of them now might lead to inconvenience.,
        ...
        while (1) 
	{
		...
		{
			get_exp_ctrl_2_neural_net_msg_type_string(msg_item.msg_type, str_exp_ctrl_msg);  
//			print_message(INFO_MSG ,"HybridNetRLBMI", "HybridNetRLBMI", "exp_ctrl_2_neural_net_msgs_handler", str_exp_ctrl_msg);
			switch (msg_item.msg_type)
			{
				case EXP_CTRL_2_NEURAL_NET_MSG_TRIAL_STATUS_CHANGED:   // current system time might be different from the one EXP_CTRL_2_NEURAL_NET_MSG_TRIAL_START. it can change during processing the message buffer but would not lead to problem.  EXP_CTRL_2_NEURAL_NET_MSG_TRIAL_STATUS_CHANGED is used by the graphs.
					...
					break;	
				case EXP_CTRL_2_NEURAL_NET_MSG_TRIAL_START:   // not implemeted yet, for RL it will be required.
...
/*					current_sys_time = rt_tasks_data->current_system_time;
					for (i = 0; i < num_of_neurons; i++)
					{
						...
						if (! write_to_neuron_trial_event_buffer(all_neurons[i], current_sys_time, NEURON_EVENT_TYPE_TRIAL_START_EVENT)) {
							print_message(ERROR_MSG ,"HybridNetRLBMI", "HybridNetRLBMI", "exp_ctrl_2_neural_net_msgs_handler", "! write_to_neuron_trial_event_buffer()."); exit(1); }	
					}					
*/					break;
...
/*					current_sys_time = rt_tasks_data->current_system_time;
					...
					if (! write_to_neural_net_2_gui_msg_buffer(msgs_neural_net_2_gui, rt_tasks_data->current_system_time, NEURAL_NET_2_GUI_MSG_UPDATE_SYNAPTIC_WEIGHTS, neural_net_2_gui_msg_add)) {
						print_message(ERROR_MSG ,"HybridNetRLBMI", "HybridNetRLBMI", "exp_ctrl_2_neural_net_msgs_handler", "! write_to_neural_net_2_gui_msg_buffer()."); exit(1); }	
*/					break;
...
/*					current_sys_time = rt_tasks_data->current_system_time;
					...
					neural_net_2_gui_msg_add.reward = msg_item.additional_data.reward;	/// THIS MESSAGE SHOULD BE SENT BEFORE EXP_CTRL_2_NEURAL_NET_MSG_STOP_RECORDING which saves the synaptic weights.
...
*/					break;
				...
				case EXP_CTRL_2_NEURAL_NET_MSG_START_RECORDING:	
					...
					if (! write_to_neural_net_2_gui_msg_buffer(msgs_neural_net_2_gui, rt_tasks_data->current_system_time, NEURAL_NET_2_GUI_MSG_START_RECORDING, neural_net_2_gui_msg_add)) {
						print_message(BUG_MSG ,"NeuralNetler", "HandleExpCtrl2NeuralNetMsgs", "write_to_neural_net_2_gui_msg_buffer", "! write_to_neural_net_2_gui_msg_buffer(().");exit(1); }	
					break;
				...
				case EXP_CTRL_2_NEURAL_NET_MSG_CANCEL_RECORDING:	
					...
					break;
				default: 
					print_message(BUG_MSG ,"HybridNetRLBMI", "HybridNetRLBMI", "exp_ctrl_2_neural_net_msgs_handler", str_exp_ctrl_msg);
					rt_make_soft_real_time();
        				rt_task_delete(handler);
					print_message(INFO_MSG ,"HybridNetRLBMI", "HybridNetRLBMI", "exp_ctrl_2_neural_net_msgs_handler", "rt_task_delete().");	
				        exit(1); 
			}
		}
		// routines	
