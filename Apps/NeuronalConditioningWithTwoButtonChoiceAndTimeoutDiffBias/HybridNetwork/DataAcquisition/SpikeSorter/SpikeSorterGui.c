...
static GtkWidget *entry_voltage_range;
static GtkWidget *btn_submit_voltage_range;
...
static void submit_voltage_range_button_func(void);
...
*/	color_spike_non_sorted.red = 65535;      // non-sorted
	...
	gtk_box_pack_start (GTK_BOX (hbox), btn_pause, TRUE, TRUE, 0);	
  	...
  	btn_submit_voltage_range = gtk_button_new_with_label("Submit Voltage Range (uV)");
	gtk_box_pack_start (GTK_BOX (hbox), btn_submit_voltage_range, TRUE, TRUE, 0);
   	
        entry_voltage_range  = gtk_entry_new();
        gtk_box_pack_start(GTK_BOX(hbox),entry_voltage_range, FALSE,FALSE,0);
	gtk_widget_set_size_request(entry_voltage_range, 50, 25) ;

	char range_str[40];
	voltage_range = HIGHEST_VOLTAGE_MV;
	sprintf(range_str, "%.1f" , voltage_range);
	gtk_entry_set_text (GTK_ENTRY(entry_voltage_range), range_str); 
	...
	g_signal_connect(G_OBJECT(btn_submit_voltage_range), "clicked", G_CALLBACK(submit_voltage_range_button_func), NULL);	
...
static void combo_mwa_func (void)
{
	...
	if (template_matching_data[disp_mwa][disp_chan][disp_unit].sorting_on)
		gtk_button_set_label (GTK_BUTTON(btn_unit_sorting_on_off),"Sorting: ON");
	else
		gtk_button_set_label (GTK_BUTTON(btn_unit_sorting_on_off),"Sorting: OFF");
	 if (template_matching_data[disp_mwa][disp_chan][disp_unit].include_unit)
		gtk_button_set_label (GTK_BUTTON(btn_include_unit_on_off),"Include: ON");
	else
		gtk_button_set_label (GTK_BUTTON(btn_include_unit_on_off),"Include: OFF");
	if (template_matching_data[disp_mwa][disp_chan][disp_unit].alarm_on)
		gtk_button_set_label (GTK_BUTTON(btn_unit_alarm_on_off),"Alarm: ON");
	else
		gtk_button_set_label (GTK_BUTTON(btn_unit_alarm_on_off),"Alarm: OFF");

	clear_spikes_screen();

	if (template_matching_data[disp_mwa][disp_chan][0].display_voltage_range > 0)

	{
		voltage_range = template_matching_data[disp_mwa][disp_chan][0].display_voltage_range;

		gtk_databox_set_total_limits (GTK_DATABOX (box_nonsorted_all_spike), 0, NUM_OF_SAMP_PER_SPIKE-1, voltage_range , -voltage_range);
		
	 	gtk_databox_set_total_limits (GTK_DATABOX (box_sorted_all_spike), 0, NUM_OF_SAMP_PER_SPIKE-1, voltage_range , -voltage_range);		

		int i;
		// Sorted Units Shape Plot		
		for (i=0; i<MAX_NUM_OF_UNIT_PER_CHAN; i++)  // including non-sorted
		{
	 		gtk_databox_set_total_limits (GTK_DATABOX (box_units[i]), 0, NUM_OF_SAMP_PER_SPIKE-1, voltage_range , -voltage_range);	 	      
		}	
	
		// Non sorted spike	

		gtk_databox_set_total_limits (GTK_DATABOX (box_non_sorted_spike), 0, NUM_OF_SAMP_PER_SPIKE-1, voltage_range , -voltage_range);	

	}
	else if (template_matching_data[disp_mwa][disp_chan][0].display_voltage_range <= 0)
		voltage_range = HIGHEST_VOLTAGE_MV;
...
static void submit_voltage_range_button_func(void)
{
	voltage_range = atof(gtk_entry_get_text(GTK_ENTRY(entry_voltage_range)));

	if (voltage_range < 100.0)
		return (void)print_message(ERROR_MSG ,"SpikeSorter", "SpikeSorter", "submit_voltage_range_button_func", "voltage_range < 100.0");
	...
	for (i = 0; i < (MAX_NUM_OF_UNIT_PER_CHAN+1); i++)
	{
		template_matching_data[disp_mwa][disp_chan][i].display_voltage_range = voltage_range;
	}
}
