...
int write_template_matching_data_v1(int num, ...)
{
	...
	if ((fp = fopen(path, "w")) == NULL)  { sprintf(message, "Couldn' t create %s.", path); print_message(ERROR_MSG ,"SpikeSorter", "FileHandler", "write_template_matching_data_v1", message); return 0; }
	...
	fprintf(fp,"	float 					display_voltage_range;\n");	
	...
	if ((fp = fopen(path, "wb")) == NULL)  { sprintf(message, "Couldn' t create %s.", path); print_message(ERROR_MSG ,"SpikeSorter", "FileHandler", "write_template_matching_data_v1", message); return 0; }
	...
	if ((fp = fopen("SpikeSorter/path_initial_directory", "r")) == NULL)  { print_message(ERROR_MSG ,"SpikeSorter", "FileHandler", "write_template_matching_data_v1", "Couldn' t read SpikeSorter/path_initial_directory."); return 0; }
	fclose(fp);
	if ((fp = fopen("SpikeSorter/path_initial_directory", "w")) == NULL)  { print_message(ERROR_MSG ,"SpikeSorter", "FileHandler", "write_template_matching_data_v1", "Couldn' t write to SpikeSorter/path_initial_directory."); return 0; }

	strcpy(path, path_folder);
	strcat(path, "/template_matching");
	fprintf(fp, "%s", path); 
	fclose (fp);	
	print_message(INFO_MSG ,"SpikeSorter", "FileHandler", "write_template_matching_data_v1", "Succesuflly saved TemplateMatching data file.");	
	return 1;
}
int read_template_matching_data_v1(int num, ...)
{
	...
	if ((fp = fopen(path, "r")) == NULL)  { sprintf(message, "Couldn' t read %s.", path); print_message(ERROR_MSG ,"SpikeSorter", "FileHandler_v1", "read_template_matching_data_v1", message); return 0; }
	if (fgets(line, sizeof line, fp ) == NULL)   { sprintf(message, "Couldn' t read %d th line of %s.", line_cntr, path); print_message(ERROR_MSG ,"SpikeSorter", "FileHandler_v1", "read_template_matching_data_v1", message);  fclose(fp); return 0; } else {line_cntr++;}   //  ----------TemplateMatching Data----------
	if (fgets(line, sizeof line, fp ) == NULL)   { sprintf(message, "Couldn' t read %d th line of %s.", line_cntr, path); print_message(ERROR_MSG ,"SpikeSorter", "FileHandler_v1", "read_template_matching_data_v1", message);  fclose(fp); return 0; } else {line_cntr++;}   //  VERSION	XX
	if (fgets(line, sizeof line, fp ) == NULL)   { sprintf(message, "Couldn' t read %d th line of %s.", line_cntr, path); print_message(ERROR_MSG ,"SpikeSorter", "FileHandler_v1", "read_template_matching_data_v1", message);  fclose(fp); return 0; } else {line_cntr++;}   //  MAX_NUM_OF_MWA	XX
	...
	if ((fp = fopen(path_bin, "rb")) == NULL)  { sprintf(message, "Couldn' t read %s.", path_bin); print_message(ERROR_MSG ,"SpikeSorter", "FileHandler_v1", "read_template_matching_data_v1", message); return 0; }	

	rewind (fp);
	if (fread (&local_template_matching_data, sizeof(TemplateMatchingData), 1, fp) != 1)  { fclose(fp);
		return print_message(ERROR_MSG ,"SpikeSorter", "SpikeSorter", "read_template_matching_data_v1", "! fread()."); }
	...
	for (i=0; i<max_num_of_mwa; i++)
	{
		for (j=0; j<max_num_of_channel_per_mwa; j++)
		{
			for (k=0; k<max_num_of_unit_per_chan; k++)
			{
				template_matching_data[i][j][k].display_voltage_range = local_template_matching_data[i][j][k].display_voltage_range;
			}
		}
	}

	print_message(INFO_MSG ,"SpikeSorter", "FileHandler", "read_template_matching_data_v1", "Succesuflly read TemplateMatching data file.");	
	return 1;
}
