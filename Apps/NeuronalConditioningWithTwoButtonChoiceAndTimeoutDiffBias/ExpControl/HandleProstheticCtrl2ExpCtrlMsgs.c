			...
			case PROSTHETIC_CTRL_2_EXP_CTRL_MSG_REWARD_REQUEST:	// Mov Obj send this out whether when it reaches target or threshold depending on its programming
				switch (*trial_status)
				{
					...
					case TRIAL_STATUS_IN_TRIAL:
						if (get_rand_number() < paradigm->lever_press_requirement_probability)
						{
							*trial_status = TRIAL_STATUS_WAIT_FOR_LEVER_PRESS;
							if (! write_to_trial_status_history(trial_status_history, current_time, TRIAL_STATUS_WAIT_FOR_LEVER_PRESS))
						...
						if (!write_to_exp_ctrl_2_trial_dur_hand_msg_buffer(msgs_exp_ctrl_2_trial_dur_hand, current_time, EXP_CTRL_2_TRIAL_DUR_HAND_MSG_ENABLE_DURATION_HANDLING, current_time + paradigm->lever_press_wait_duration))
							return print_message(ERROR_MSG ,"ExpControl", "HandleExpCtrlDur2ExpCtrlMsgss", "handle_trial_dur_handler_to_exp_control_msg", "write_to_exp_ctrl_2_trial_dur_hand_msg_buffer()");
