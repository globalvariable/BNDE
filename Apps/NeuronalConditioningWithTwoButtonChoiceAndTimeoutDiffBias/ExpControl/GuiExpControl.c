...
static GtkWidget *btn_punishment;

static GtkWidget *entry_trial_length;
static GtkWidget *btn_submit_trial_length;

static GtkWidget *entry_lever_press_requirement_probability;
static GtkWidget *btn_submit_lever_press_requirement_probability;

static GtkWidget *entry_lever_press_wait_duration;
static GtkWidget *btn_submit_lever_press_wait_duration;
...
static void punishment_button_func (void);

static void start_trial_button_func (void);

static void submit_trial_length_button_func (void);

static void submit_lever_press_requirement_probability_button_func (void);

static void submit_lever_press_wait_duration_button_func (void);
...
bool create_exp_control_tab(GtkWidget *tabs, RtTasksData *rt_tasks_data, Gui2ExpCtrlMsg *msgs_gui_2_exp_ctrl, ExpCtrlParadigmRobotReach *exp_ctrl_paradigm, ClassifiedExpHistory* classified_trial_history, ExpCtrl2GuiMsg *msgs_exp_ctrl_2_gui, ExpStatusHistory *trial_status_history)
{
	...
	static_trial_status_history = trial_status_history;
        ...
        frame = gtk_frame_new ("");
        frame_label = gtk_label_new ("     Exps Handler    ");      
	...
	btn_punishment = gtk_button_new_with_label("Punishment");
	gtk_box_pack_start (GTK_BOX (hbox), btn_punishment , TRUE, TRUE, 0);
	...
	btn_start_trial = gtk_button_new_with_label("Start Exp");
	...
	lbl = gtk_label_new("Lever Press Req. Probab.: ");
        gtk_box_pack_start(GTK_BOX(hbox),lbl, FALSE, FALSE, 0);
	entry_lever_press_requirement_probability = gtk_entry_new();
        gtk_box_pack_start(GTK_BOX(hbox), entry_lever_press_requirement_probability, FALSE, FALSE, 0);
	gtk_entry_set_text(GTK_ENTRY(entry_lever_press_requirement_probability), "1.0");

	hbox = gtk_hbox_new(FALSE, 0);
	gtk_box_pack_start(GTK_BOX(vbox),hbox, FALSE,FALSE,0);

	btn_submit_lever_press_requirement_probability = gtk_button_new_with_label("Submit");
	gtk_box_pack_start (GTK_BOX (hbox), btn_submit_lever_press_requirement_probability , TRUE, TRUE, 0);
	...
	lbl = gtk_label_new("Lever Press Wait Dur (ms): ");
        gtk_box_pack_start(GTK_BOX(hbox),lbl, FALSE, FALSE, 0);
	entry_lever_press_wait_duration = gtk_entry_new();
        gtk_box_pack_start(GTK_BOX(hbox), entry_lever_press_wait_duration, FALSE, FALSE, 0);
	gtk_entry_set_text(GTK_ENTRY(entry_lever_press_wait_duration), "1000");

	hbox = gtk_hbox_new(FALSE, 0);
	gtk_box_pack_start(GTK_BOX(vbox),hbox, FALSE,FALSE,0);

	btn_submit_lever_press_wait_duration = gtk_button_new_with_label("Submit");
	gtk_box_pack_start (GTK_BOX (hbox), btn_submit_lever_press_wait_duration , TRUE, TRUE, 0);
	...
	lbl = gtk_label_new("---- Interrogate Exp ----");
        gtk_box_pack_start(GTK_BOX(hbox),lbl, TRUE, TRUE, 0);

	hbox = gtk_hbox_new(FALSE, 0);
        gtk_box_pack_start(GTK_BOX(vbox),hbox, FALSE,FALSE,0);
	lbl = gtk_label_new("Exp #: ");
        gtk_box_pack_start(GTK_BOX(hbox),lbl, FALSE, FALSE, 0);
	lbl = gtk_label_new("");
        gtk_box_pack_start(GTK_BOX(hbox),lbl, TRUE, TRUE, 0);
	...
	btn_submit_trial_number = gtk_button_new_with_label("Submit Exp Number");
	...
	g_signal_connect(G_OBJECT(btn_punishment), "clicked", G_CALLBACK(punishment_button_func), NULL);
	...
	g_signal_connect(G_OBJECT(btn_submit_lever_press_requirement_probability), "clicked", G_CALLBACK(submit_lever_press_requirement_probability_button_func), NULL);

	g_signal_connect(G_OBJECT(btn_submit_lever_press_wait_duration), "clicked", G_CALLBACK(submit_lever_press_wait_duration_button_func), NULL);
...
static gboolean timeout_callback(gpointer user_data) 
{
	...
	while (get_next_exp_ctrl_2_gui_msg_buffer_item(static_msgs_exp_ctrl_2_gui, &msg_item))
	{
		switch (msg_item.msg_type)
		{
			case EXP_CTRL_2_GUI_MSG_BROADCAST_START_RECORDING_MSG_ACK:
				...
				{
					print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "timeout_callback", " *create_data_directory().");	
					exit(1);
				}
				...
				{
					print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "timeout_callback", " *write_to_data_files().");		
					exit(1);
				}	
			...
			case EXP_CTRL_2_GUI_MSG_BROADCAST_STOP_RECORDING_MSG_ACK:
				...
				{
					print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "timeout_callback", " *fclose_all_data_files().");
					exit(1);
				}
				...
				{
					if (!write_to_gui_2_exp_ctrl_msg_buffer(static_msgs_gui_2_exp_ctrl, static_rt_tasks_data->current_system_time, GUI_2_EXP_CTRL_MSG_BROADCAST_START_RECORDING, 0))
						return print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "enable_trials_button_func", "! write_to_gui_2_exp_ctrl_msg_buffer().");			
				}
				break;
			case EXP_CTRL_2_GUI_MSG_BROADCAST_CANCEL_RECORDING_MSG_ACK:
				...
				{
					print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "timeout_callback", " *fdelete_all_data_files().");
					exit(1);
				}
			...
			case EXP_CTRL_2_GUI_MSG_TRIAL_STATUS_CHANGE:
				...
				switch (trial_status)
				{
					...
					case TRIAL_STATUS_IN_REFRACTORY:
						...
						{
							usleep(100000);
							if (!write_to_gui_2_exp_ctrl_msg_buffer(static_msgs_gui_2_exp_ctrl, static_rt_tasks_data->current_system_time, GUI_2_EXP_CTRL_MSG_BROADCAST_STOP_RECORDING, 0))
								return print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "enable_trials_button_func", "! write_to_gui_2_exp_ctrl_msg_buffer().");	
						}
						else
						{
					...
					case TRIAL_STATUS_GET_READY_TO_START:
						break;
					default:
						return print_message(BUG_MSG ,"ExpControl", "GuiExpControl", "timeout_callback", "switch (trial_status) - default");
				}
				break;
			default:
				return print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "timeout_callback", "switch (msg_item.msg_type) - default");
		}
	}
	if (recording)
	{
...
static void enable_trials_button_func (void)
{
	if (!write_to_gui_2_exp_ctrl_msg_buffer(static_msgs_gui_2_exp_ctrl, static_rt_tasks_data->current_system_time, GUI_2_EXP_CTRL_MSG_ENABLE_EXP_CTRLLING, 0))
		return (void)print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "enable_trials_button_func", "! write_to_gui_2_exp_ctrl_msg_buffer().");		
	gtk_widget_set_sensitive(btn_disable_trials, TRUE);	
}

static void disable_trials_button_func (void)
{
	if (!write_to_gui_2_exp_ctrl_msg_buffer(static_msgs_gui_2_exp_ctrl, static_rt_tasks_data->current_system_time, GUI_2_EXP_CTRL_MSG_DISABLE_EXP_CTRLLING, 0))
		return (void)print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "disable_trials_button_func", "! write_to_gui_2_exp_ctrl_msg_buffer().");		
}

static void quit_trials_button_func (void)
{
	if (!write_to_gui_2_exp_ctrl_msg_buffer(static_msgs_gui_2_exp_ctrl, static_rt_tasks_data->current_system_time, GUI_2_EXP_CTRL_MSG_QUIT, 0))
		return (void)print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "quit_trials_button_func", "! write_to_gui_2_exp_ctrl_msg_buffer().");		
}

static void auto_target_select_mode_on_button_func(void)
{
	if (!write_to_gui_2_exp_ctrl_msg_buffer(static_msgs_gui_2_exp_ctrl, static_rt_tasks_data->current_system_time, GUI_2_EXP_CTRL_MSG_AUTO_TARGET_SELECTION_ON, 0))
		return (void)print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "auto_target_select_mode_on_button_func", "! write_to_gui_2_exp_ctrl_msg_buffer().");	
}

static void auto_target_select_mode_off_button_func(void)
{
	if (!write_to_gui_2_exp_ctrl_msg_buffer(static_msgs_gui_2_exp_ctrl, static_rt_tasks_data->current_system_time, GUI_2_EXP_CTRL_MSG_AUTO_TARGET_SELECTION_OFF, 0))
		return (void)print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "auto_target_select_mode_off_button_func", "! write_to_gui_2_exp_ctrl_msg_buffer().");	
}
...
static void select_target_button_func (void)
{
	...
	if (target_idx >= paradigm->num_of_robot_target_positions)
		return (void)print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "select_target_button_func", "target_idx >= static_exp_ctrl_paradigm->num_of_robot_target_positions.");	
	if (!write_to_gui_2_exp_ctrl_msg_buffer(static_msgs_gui_2_exp_ctrl, static_rt_tasks_data->current_system_time, GUI_2_EXP_CTRL_MSG_SELECT_TARGET, target_idx))
		return (void)print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "select_target_button_func", "! write_to_gui_2_exp_ctrl_msg_buffer().");	
}


static void increase_robot_start_idx_button_func (void)
{
	if (!write_to_gui_2_exp_ctrl_msg_buffer(static_msgs_gui_2_exp_ctrl, static_rt_tasks_data->current_system_time, GUI_2_EXP_CTRL_MSG_INCREASE_ROBOT_START_POSITION_IDX, 0))
		return (void)print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "increase_robot_start_idx_button_func", "! write_to_gui_2_exp_ctrl_msg_buffer().");		
}

static void decrease_robot_start_idx_button_func (void)
{
	if (!write_to_gui_2_exp_ctrl_msg_buffer(static_msgs_gui_2_exp_ctrl, static_rt_tasks_data->current_system_time, GUI_2_EXP_CTRL_MSG_DECREASE_ROBOT_START_POSITION_IDX, 0))
		return (void)print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "decrease_robot_start_idx_button_func", "! write_to_gui_2_exp_ctrl_msg_buffer().");		
}
...
static void submit_trial_number_button_func (void)
{
	...
	if (trial_number >= classified_history->all_trials->buffer_size)
		return (void)print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "disable_trials_button_func", "Invalid Exp Number.");	
...
static void create_recording_folder_button_func (void)
{
	...
	if (strcmp(&(path_temp[path_len-8]),"EXP_DATA") == 0)
		return (void)print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "create_recording_folder_button_func", "Selected folder is /EXP_DATA main folder. Select a folder inside this folder.");				
	if ((*create_main_directory[MAX_NUMBER_OF_DATA_FORMAT_VER-1])(2, path, paradigm))		// record in last format version
	{
		
	}
	else
		print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "create_recording_folder_button_func", " *create_main_directory().");				
}
...
static void cancel_recording_button_func (void)
{
	if (!write_to_gui_2_exp_ctrl_msg_buffer(static_msgs_gui_2_exp_ctrl, static_rt_tasks_data->current_system_time, GUI_2_EXP_CTRL_MSG_BROADCAST_CANCEL_RECORDING, 0))
		return (void)print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "enable_trials_button_func", "! write_to_gui_2_exp_ctrl_msg_buffer().");	
}
...
static void set_directory_btn_select_directory_to_save(void)
{
	...
	FILE *fp = NULL;
       	...
       	{ 
		print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "set_directory_btn_select_directory_to_save", "Couldn't find file: ./path_initial_directory.");
		print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "set_directory_btn_select_directory_to_save", "/home is loaded as initial direcoty to create data folder.");
		...
		{ 
			print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "set_directory_btn_select_directory_to_save", "Couldn' t read ./path_initial_directory.");
			print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "set_directory_btn_select_directory_to_save", "/home is loaded as initial direcoty to create data folder.");
			gtk_file_chooser_set_current_folder (GTK_FILE_CHOOSER (btn_select_directory_to_save),"/home");
		}
		else
...
static void release_reward_button_func (void)
{
	if (!write_to_gui_2_exp_ctrl_msg_buffer(static_msgs_gui_2_exp_ctrl, static_rt_tasks_data->current_system_time, GUI_2_EXP_CTRL_MSG_RELEASE_REWARD, 0))
		return (void)print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "start_recording_button_func ", "! write_to_gui_2_exp_ctrl_msg_buffer().");	
}

static void punishment_button_func (void)
{
	if (!write_to_gui_2_exp_ctrl_msg_buffer(static_msgs_gui_2_exp_ctrl, static_rt_tasks_data->current_system_time, GUI_2_EXP_CTRL_MSG_PUNISHMENT_REQUEST, 0))
		return (void)print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "start_recording_button_func ", "! write_to_gui_2_exp_ctrl_msg_buffer().");	
}
...
static void start_trial_button_func (void)
{
	...
	if (!write_to_gui_2_exp_ctrl_msg_buffer(static_msgs_gui_2_exp_ctrl, static_rt_tasks_data->current_system_time, GUI_2_EXP_CTRL_MSG_START_TRIAL_REQUEST, 0))
		return (void)print_message(ERROR_MSG ,"ExpControl", "GuiExpControl", "start_recording_button_func ", "! write_to_gui_2_exp_ctrl_msg_buffer().");	
...
{
	unsigned int i;
	paradigm->lever_press_requirement_probability = atof(gtk_entry_get_text(GTK_ENTRY(entry_lever_press_requirement_probability)));
	printf ("Lever Press Requirement Probability: %f\n", paradigm->lever_press_requirement_probability);
}
...
{
	unsigned int i;
	paradigm->lever_press_wait_duration = 1000000* (TimeStamp)atof(gtk_entry_get_text(GTK_ENTRY(entry_lever_press_wait_duration)));
	printf ("lever_press_wait_duration: %llu\n", paradigm->lever_press_wait_duration);
}
