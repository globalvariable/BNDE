...
static int create_main_meta_file(char *main_directory_path, ExpCtrlParadigmRobotReach *paradigm);
...
static int write_to_trial_status_data(ExpStatusHistory *trial_status_history);
static int close_meta_data(TimeStamp rec_end);
static int close_trial_data(ExpData *trial_data);
...
int create_main_directory_v0(int num, ...)
{
	FILE *fp;
	char *path_chooser;
  	ExpCtrlParadigmRobotReach *paradigm;
	DIR	*dir_main_folder;
	...
	paradigm = va_arg ( arguments, ExpCtrlParadigmRobotReach *); 
	va_end ( arguments );
	
	strcpy(main_directory_path, path_chooser);	
	strcat(main_directory_path, "/ExpControlRecord");
	if ((dir_main_folder = opendir(main_directory_path)) != NULL)
        {
        	printf ("ExpControl: ERROR: path: %s already has ExpControlRecord folder.\n", path_chooser);		
        	printf ("ExpControl: ERROR: Select another folder or delete ExpControlRecord directory.\n\n");	
	...
	mkdir(main_directory_path, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH | S_IWOTH);
        ...
        printf ("ExpControl: Created /ExpControlRecord folder in: %s.\n", path_chooser);
        printf ("ExpControl: /ExpControlRecord path is: %s.\n", main_directory_path); 
        
	if (!create_main_meta_file(main_directory_path, paradigm))
		return 0;

	if ((fp = fopen("./path_initial_directory", "w")) == NULL)  { printf ("ERROR: ExpControl: Couldn't find directory: %s\n\n", "./path_initial_directory"); return 0; }
...
int create_data_directory_v0(int num, ...)
{
	...
	{
		printf("ExpControl: ERROR: data directory counter is %d.\n", data_directory_cntr);
		printf("ExpControl: ERROR: Supported range is 0<= x <100000.\n\n");		
		return 0;
	}
	
	strcpy(data_directory_path, path_chooser);	
	strcat(data_directory_path, "/ExpControlRecord/");
	strcat(data_directory_path, data_directory_name);	
	if ((dir_data_directory = opendir(data_directory_path)) != NULL)
        {
        	printf ("ExpControl: ERROR: path: %s already has %s folder.\n", data_directory_path, data_directory_name);		
	...
	if ((ret = mkdir(data_directory_path, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH | S_IWOTH)) != 0)
	{
		...
		return print_message(ERROR_MSG ,"ExpControl", "DataFormat_v0", "create_data_directory_v0", "mkdir() != 0");
	}	

	if (! create_data_files(rec_start, data_directory_path))
		return print_message(ERROR_MSG ,"ExpControl", "DataFormat_v0", "create_data_directory_v0", "! create_data_files.");
...
int fclose_all_data_files_v0(int num, ...)
{
	...
	trial_data = va_arg ( arguments, ExpData*); 
	va_end ( arguments );	

	if (! close_meta_data(rec_end))
		return print_message(ERROR_MSG ,"ExpControl", "DataFormat_v0", " fclose_all_data_files_v0", "! close_meta_data");

	if (! close_trial_data(trial_data))
		return print_message(ERROR_MSG ,"ExpControl", "DataFormat_v0", " fclose_all_data_files_v0", "! close_meta_data");
	if (! close_trial_status_data())
		return print_message(ERROR_MSG ,"ExpControl", "DataFormat_v0", " fclose_all_data_files_v0", "! close_trial_status_data");
...
int delete_data_directory_v0(int num, ...)   // call it after fclose_all_data_files_v0
{
	...
	if ((dir_data_directory = opendir(data_directory_path)) == NULL)
        {
        	printf ("ExpControl: ERROR: path: %s does not have %s folder.\n", data_directory_path, data_directory_name);		
		closedir(dir_data_directory);
                return 0;
        }
	closedir(dir_data_directory);	 
	...
	if (! delete_data_files(data_directory_path))
		return print_message(ERROR_MSG ,"ExpControl", "DataFormat_v0", "delete_data_directory_v0", "! delete_data_files(data_directory_path)");

	if (rmdir(data_directory_path) != 0) 
		return print_message(ERROR_MSG ,"ExpControl", "DataFormat_v0", "delete_data_directory_v0", "! remove(data_directory_path)");
...
{
	ExpStatusHistory *trial_status_history;
  	va_list arguments;
	va_start ( arguments, num );   
    	trial_status_history = va_arg ( arguments, ExpStatusHistory *); 
	va_end ( arguments );

	if (! write_to_trial_status_data(trial_status_history))
		return print_message(ERROR_MSG ,"ExpControl", "DataFormat_v0", "write_to_data_files_v0", "! write_to_trial_status_data()");
...
static int create_main_meta_file(char *main_directory_path, ExpCtrlParadigmRobotReach *paradigm)
{
	...
	if ((fp = fopen(temp_path, "w")) == NULL)  { printf ("ERROR: ExpControl: Couldn't create file: %s\n\n", temp_path); return 0; }
		
	fprintf(fp,"----------ExpControl - Main Meta File----------\n");
	...
	fprintf(fp,"----------ExpControl - End of Main Meta File----------\n");
	fclose(fp);
	return 1;
}
...
static int create_data_files(TimeStamp rec_start, char *data_directory_path)
{
	if (!create_meta_data(rec_start, data_directory_path))
		return print_message(ERROR_MSG ,"ExpControl", "DataFormat_v0", "create_data_files", "! create_meta_data");
	
	if (!create_trial_data(data_directory_path))
		return print_message(ERROR_MSG ,"ExpControl", "DataFormat_v0", "create_data_files", "! create_trial_data");
...
static int create_meta_data(TimeStamp rec_start, char *data_directory_path)
{
	...
	strcat(temp, "/meta");
	if ((fp = fopen(temp, "w")) == NULL)  { printf ("ERROR: ExpControl: Couldn't create file: %s\n\n", temp); return 0; }
	fprintf(fp,"----------ExpControl - Meta File----------\n");
	time ( &rawtime );
	timeinfo = localtime (&rawtime);
...
static int create_trial_data(char *data_directory_path)
{
	...
	if ((fp = fopen(temp, "w")) == NULL)  { printf ("ERROR: ExpControl: Couldn't create file: %s\n\n", temp); return 0; }
	fprintf(fp,"----------ExpControl - ExpData File----------\n");
...
static int create_trial_status_data(char *data_directory_path)
{
	...
	if ((fp = fopen(temp, "w")) == NULL)  { printf ("ERROR: ExpControl: Couldn't create file: %s\n\n", temp); return 0; }
	fprintf(fp,"----------ExpControl - ExpStatus File----------\n");
...
static int close_meta_data(TimeStamp rec_end)
{
	...
	fprintf(fp,"----------ExpControl - End of Meta File----------\n");
	fclose(fp);
	return 1;
}
...
static int close_trial_data(ExpData *trial_data)
{
	...
	fprintf(fp,"lever_press_requirement_probability\t%f\n", trial_data->lever_press_requirement_probability);
	fprintf(fp,"lever_press_wait_duration\t%llu\n", trial_data->lever_press_wait_duration);
	fprintf(fp,"----------ExpControl - End of ExpData File----------\n");
	fclose(fp);
	return 1;
}
...
static int close_trial_status_data(void)
{
	...
	fprintf(fp,"----------ExpControl - End of ExpStatus File----------\n");
	fclose(fp);
	return 1;
}
...
static int delete_data_files(char *data_directory_path)
{
	if (! delete_meta_data(data_directory_path))
		return print_message(ERROR_MSG ,"ExpControl", "DataFormat_v0", "delete_data_files_v0", "! delete_meta_data(data_directory_path)");
	
	if (! delete_trial_data(data_directory_path))
		return print_message(ERROR_MSG ,"ExpControl", "DataFormat_v0", "delete_data_files_v0", "! delete_trial_data(data_directory_path)");

	if (! delete_trial_status_data(data_directory_path))
		return print_message(ERROR_MSG ,"ExpControl", "DataFormat_v0", "delete_data_files_v0", "! delete_trial_status_data(data_directory_path)");
...
static int delete_meta_data(char *data_directory_path)
{
	...
	if (remove(temp) != 0)  { printf ("ERROR: ExpControl: Couldn't delete file: %s\n\n", temp); return 0; }
