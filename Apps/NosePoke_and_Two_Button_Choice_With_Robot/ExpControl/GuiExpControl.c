			...
			case EXP_CTRL_2_GUI_MSG_TRIAL_STATUS_CHANGE:
				...
				switch (trial_status)
				{
					...
					case TRIAL_STATUS_IN_REFRACTORY:
						...
						sprintf (temp, "%.2f", averaging_struct_get_mean(paradigm->target_success_average_small[0]));
						gtk_label_set_text (GTK_LABEL (lbl_success_ratio_left_small), temp);

						sprintf (temp, "%u", classified_history->trial_types[1]->buff_write_idx);
						gtk_label_set_text (GTK_LABEL (lbl_num_of_right_trials), temp);
						sprintf (temp, "%.2f", averaging_struct_get_mean(paradigm->target_success_average_small[1]));
						gtk_label_set_text (GTK_LABEL (lbl_success_ratio_right_small), temp);
