...
static GtkWidget *btn_connect_to_left;
static GtkWidget *btn_connect_to_right;
...
static GtkWidget *entry_external_neurons_inhibitory_plastic_synapse;static GtkWidget *combo_mwa;
static GtkWidget *combo_chan;
static GtkWidget *combo_unit;
...
static void connect_to_left_button_func(void);
static void connect_to_right_button_func(void);

static void start_button_func(void);
...
bool create_unit_groups_gui(GtkWidget *tabs)
{
	...
	static_tabs = tabs;
        ...
        frame = gtk_frame_new ("");
        frame_label = gtk_label_new ("     Unit Groups     ");      
...
///////////////////////////////////////////// FIRST COLUMN  ///////////////////////////////////////////////////////////////
	...
	lbl = gtk_label_new("Select Unit: ");
        gtk_box_pack_start(GTK_BOX(hbox),lbl, FALSE,FALSE,20);
	...
	lbl = gtk_label_new("Connect Neurons: ");
        gtk_box_pack_start(GTK_BOX(hbox),lbl, FALSE,FALSE,0);

  	hbox = gtk_hbox_new(TRUE, 0);
        gtk_box_pack_start(GTK_BOX(vbox),hbox, FALSE,FALSE,20);

	btn_connect_to_left = gtk_button_new_with_label("   LEFT   ");
        gtk_box_pack_start(GTK_BOX(hbox),btn_connect_to_left, TRUE,TRUE,0);
	lbl = gtk_label_new("");
        gtk_box_pack_start(GTK_BOX(hbox),lbl, TRUE,TRUE,0);
	btn_connect_to_right = gtk_button_new_with_label("   RIGHT   ");
        gtk_box_pack_start(GTK_BOX(hbox),btn_connect_to_right, TRUE,TRUE,0);
...
/*	combo_neuron_type = gtk_combo_box_new_text();
	...
	lbl = gtk_label_new("");
        ...
        entry_C = gtk_entry_new();
        gtk_box_pack_start(GTK_BOX(hbox),entry_C, FALSE,FALSE,0);
	gtk_widget_set_size_request(entry_C, 50, 25) ;	g_signal_connect(G_OBJECT(combo_mwa), "changed", G_CALLBACK(combo_mwa_func), NULL);
	g_signal_connect(G_OBJECT(combo_chan), "changed", G_CALLBACK(combo_chan_func), NULL);
	g_signal_connect(G_OBJECT(combo_unit), "changed", G_CALLBACK(combo_unit_func), NULL);
	...
	lbl = gtk_label_new("v_rest  : ");	g_signal_connect(G_OBJECT(combo_mwa), "changed", G_CALLBACK(combo_mwa_func), NULL);
	g_signal_connect(G_OBJECT(combo_chan), "changed", G_CALLBACK(combo_chan_func), NULL);
	g_signal_connect(G_OBJECT(combo_unit), "changed", G_CALLBACK(combo_unit_func), NULL);
        gtk_box_pack_start(GTK_BOX(hbox),lbl, FALSE,FALSE,0);
	lbl = gtk_label_new("");
	...
	lbl = gtk_label_new(" Tau_Inh  : ");
        gtk_box_pack_st	btn_make_output = gtk_button_new_with_label("   Make Output   ");
        gtk_box_pack_start(GTK_BOX(hbox),btn_make_output, FALSE,FALSE,0);
	lbl = gtk_label_new("");
        gtk_box_pack_start(GTK_BOX(hbox),lbl, TRUE,TRUE,0);art(GTK_BOX(hbox),lbl, FALSE,FALSE,0);
	lbl = gtk_label_new("");
        gtk_box_pack_start(GTK_BOX(hbox),lbl, TRUE,TRUE,0);
	...
	gtk_widget_set_size_request(entry_num_of_neuron_for_group, 50, 25) ;
  	...
  	hbox = gtk_hbox_new(FALSE, 0);	g_signal_connect(G_OBJECT(combo_mwa), "changed", G_CALLBACK(combo_mwa_func), NULL);
	g_signal_connect(G_OBJECT(combo_chan), "changed", G_CALLBACK(combo_chan_func), NULL);
	g_signal_connect(G_OBJECT(combo_unit), "changed", G_CALLBACK(combo_unit_func), NULL);
        gtk_box_pack_start(GTK_BOX(vbox),hbox, FALSE,FALSE,0);
	...
	gtk_widget_set_size_request(entry_parker_sochacki_err_tol, 65, 25) ;	g_signal_connect(G_OBJECT(combo_mwa), "changed", G_CALLBACK(combo_mwa_func), NULL);
	g_signal_connect(G_OBJECT(combo_chan), "changed", G_CALLBACK(combo_chan_func), NULL);
	g_signal_connect(G_OBJECT(combo_unit), "changed", G_CALLBACK(combo_unit_func), NULL);
	...
	gtk_box_pack_start (GTK_BOX (hbox), btn_interrogate_neuron, TRUE, TRUE, 0);	g_signal_connect(G_OBJECT(combo_mwa), "changed", G_CALLBACK(combo_mwa_func), NULL);
	g_signal_connect(G_OBJECT(combo_chan), "changed", G_CALLBACK(combo_chan_func), NULL);
	g_signal_connect(G_OBJECT(combo_unit), "changed", G_CALLBACK(combo_unit_func), NULL);
...
///////////////////////////////////////////// SECOND COLUMN  ///////////////////////////////////////////////////////////////
	...
	vbox = gtk_vbox_new(FALSE, 0);
	gtk_table_attach_defaults(GTK_TABLE(table), vbox, 1,2, 0, 3);  
...
/////////  THIRD & FOURTH COLUMN  ////////////////////////////////
	...
	gtk_table_attach_defaults(GTK_TABLE(table), vbox, 2,4, 0, 3);
...
////    SIGNALS   ///////////////////////////
	...
	g_signal_connect(G_OBJECT(btn_connect_to_left), "clicked", G_CALLBACK(connect_to_left_button_func), NULL);
	g_signal_connect(G_OBJECT(btn_connect_to_right), "clicked", G_CALLBACK(connect_to_right_button_func), NULL);

	g_signal_connect(G_OBJECT(btn_start), "clicked", G_CALLBACK(start_button_func), NULL);
...
{
	Neuron *nrn;
	nrn = get_neuron_address (blue_spike_network, disp_mwa, disp_chan, disp_unit);

	if (!nrn->include)
		return print_message(ERROR_MSG ,"HybridNetwork", "NetworkView", "connect_to_left_button_func", "This neuron is not included in spiker sorter");


	if (! connect_neurons(blue_spike_network, disp_mwa, disp_chan, disp_unit, in_silico_network, 0, 0, 0, BLUESPIKE_2_IN_SILICO_EXCITATORY_WEIGHT_MAX, BLUESPIKE_2_IN_SILICO_EXCITATORY_WEIGHT_MIN, BLUESPIKE_2_IN_SILICO_INHIBITORY_WEIGHT_MAX, BLUESPIKE_2_IN_SILICO_INHIBITORY_WEIGHT_MIN, BLUESPIKE_2_IN_SILICO_EXCITATORY_DELAY_MAX, BLUESPIKE_2_IN_SILICO_EXCITATORY_DELAY_MIN, BLUESPIKE_2_IN_SILICO_INHIBITORY_DELAY_MAX, BLUESPIKE_2_IN_SILICO_INHIBITORY_DELAY_MIN, MAXIMUM_BLUE_SPIKE_TO_IN_SILICO_AXONAL_DELAY, MINIMUM_BLUE_SPIKE_TO_IN_SILICO_AXONAL_DELAY, FALSE, FALSE))
		return print_message(ERROR_MSG ,"HybridNetwork", "NetworkView", "connect_to_left_button_func", "! connect_neurons().");

	printf ("mwa:%d chan:%d unit:%d connected to LEFT direction.\n", disp_mwa, disp_chan, disp_unit);
...
static void connect_to_right_button_func (void)
{
	...
	if (! connect_neurons(blue_spike_network, disp_mwa, disp_chan, disp_unit, in_silico_network, 1, 0, 0, BLUESPIKE_2_IN_SILICO_EXCITATORY_WEIGHT_MAX, BLUESPIKE_2_IN_SILICO_EXCITATORY_WEIGHT_MIN, BLUESPIKE_2_IN_SILICO_INHIBITORY_WEIGHT_MAX, BLUESPIKE_2_IN_SILICO_INHIBITORY_WEIGHT_MIN, BLUESPIKE_2_IN_SILICO_EXCITATORY_DELAY_MAX, BLUESPIKE_2_IN_SILICO_EXCITATORY_DELAY_MIN, BLUESPIKE_2_IN_SILICO_INHIBITORY_DELAY_MAX, BLUESPIKE_2_IN_SILICO_INHIBITORY_DELAY_MIN, MAXIMUM_BLUE_SPIKE_TO_IN_SILICO_AXONAL_DELAY, MINIMUM_BLUE_SPIKE_TO_IN_SILICO_AXONAL_DELAY, FALSE, FALSE))
		return print_message(ERROR_MSG ,"HybridNetwork", "NetworkView", "connect_to_right_button_func", "! connect_neurons().");

	printf ("mwa:%d chan:%d unit:%d connected to RIGHT direction.\n", disp_mwa, disp_chan, disp_unit);
}
