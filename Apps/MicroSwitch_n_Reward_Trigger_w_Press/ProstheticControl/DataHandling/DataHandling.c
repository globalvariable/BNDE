...
int is_prosthetic_control_data(char *prosthetic_control_data_path)
{
	...
	FILE *fp;
	int line_cntr = 0;
	strcpy(path, prosthetic_control_data_path);
	strcat(path, "/meta");
	if ((fp = fopen(path, "r")) == NULL)  { printf ("ERROR: ProstheticControl: Couldn't read file: %s\n\n", path); return 0; }
	...
	fclose(fp);   
	if (strcmp(line, "----------ProstheticControl - Main Meta File----------\n" ) == 0)
		return 1;
	else
		return 0;	
}
...
int get_format_version(int *version, char *prosthetic_control_data_path)
{
	...
	int line_cntr = 0;
	FILE *fp;
	if (is_prosthetic_control_data(prosthetic_control_data_path))
	{
		strcpy(path, prosthetic_control_data_path);
		strcat(path, "/meta");		
