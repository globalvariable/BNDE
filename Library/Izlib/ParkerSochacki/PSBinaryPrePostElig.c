...
static void eligibility_tag_synapses (Neuron *nrn, RTIME spike_time);



bool evaluate_neuron_dyn_pre_post_w_binary_stdp_elig(Neuron *nrn, TimeStamp start_time, TimeStamp end_time, bool *spike_generated, TimeStamp *spike_time)	// Assume that only once a spike can be generated per step (0.25 ms).
{			
...
//		printf("event: %llu syn: %u weight: %f type: %u\n", event_time, event_item->syn_idx, synapses[event_item->syn_idx].weight, event_item->event_type); 	
		if (event_item->event_type == NEURON_EVENT_TYPE_SYNAPTIC_EVENT)
		{
			...
			if (synapse->plastic)
			{
				synapse->ps_stdp_pre_post->last_set_time = event_time; 
			}
		}
		else
		{
...
//		printf("---------------->  Spike time %.15f %llu\n", ((integration_start_time)/PARKER_SOCHACKI_EMBEDDED_STEP_SIZE)+dt_part, *spike_time);		
		...
		eligibility_tag_synapses (nrn, *spike_time); 
...
static void eligibility_tag_synapses (Neuron *nrn, RTIME spike_time)
{
	...
	for (i = 0; i < num_of_synapses; i++)
	{
		if (synapses[i].plastic)
		{
			...
			if ((synapse->ps_stdp_pre_post->last_set_time + synapse->ps_stdp_pre_post->duration) > spike_time)
			{
				synapse->ps_eligibility->last_set_time = spike_time;
			}
		}
	}
	return;
