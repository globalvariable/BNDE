...
EligibilityGraphScrollLimited* allocate_eligibility_graph_scroll_limited(GtkWidget *hbox, EligibilityGraphScrollLimited *graph, unsigned int num_of_data_points, TimeStamp sampling_interval, unsigned int num_of_data_points_to_scroll, TimeStamp buffer_followup_latency, unsigned int num_of_markers, ExpStatusEvents *trial_status_events, EligibilityBufferLimited* limited_eligibility_buffer, unsigned int limited_eligibility_buffer_list_idx)
{
	unsigned int i;
	if (graph != NULL)
	{
