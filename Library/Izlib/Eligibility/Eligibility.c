...
bool create_ps_eligibility_for_neuron_group(Network *network, unsigned int layer, unsigned int nrn_grp, RTIME duration)
{
	...
	for (i = 0; i < num_of_neurons; i++)
	{
		...
		if (! create_ps_eligibility_for_neuron(neuron, duration))
...
bool create_ps_eligibility_for_neuron(Neuron* neuron, RTIME duration)
{
	...
	if (duration < 0)
		return print_message(BUG_MSG ,"IzNeuronSimulators", "Eligibility", "create_ps_eligibility_for_neuron", "duration < 0.");
	...
	for (i = 0; i < num_of_synapses; i++)
	{
		if (neuron->syn_list->synapses[i].plastic)
		{
			...
			neuron->syn_list->synapses[i].ps_eligibility->duration = duration ;
		}
	}
...
bool create_ps_eligibility_for_synapse(Neuron* neuron, RTIME duration, unsigned int synapse)
{
	...
	if (duration < 0)
		return print_message(BUG_MSG ,"IzNeuronSimulators", "Eligibility", "create_ps_eligibility_for_synapse", "duration < 0.");
...
bool submit_new_ps_eligibility_vals_for_neuron(Neuron* neuron, RTIME duration)
{
	...
	if (duration < 0)
		return print_message(BUG_MSG ,"IzNeuronSimulators", "Eligibility", "submit_new_ps_eligibility_vals_for_neuron", "duration < 0.");
...
bool submit_new_ps_eligibility_vals_for_synapse(Neuron* neuron, RTIME duration, unsigned int synapse)
{
	...
	if (duration < 0)
		return print_message(BUG_MSG ,"IzNeuronSimulators", "Eligibility", "submit_new_ps_eligibility_vals_for_synapse", "duration < 0.");
...
bool submit_new_ps_eligibility_vals_for_neuron_according_to_pre_synaptic_neuron_group(Neuron* neuron, Network *axon_from_network, unsigned int axon_from_layer, unsigned int axon_from_nrn_grp, RTIME duration)
{
	...
	if (duration < 0)
		return print_message(BUG_MSG ,"IzNeuronSimulators", "Eligibility", "submit_new_ps_eligibility_vals_for_neuron_according_to_pre_synaptic_neuron_group", "duration < 0.");
